class obus extends Phaser.Physics.Arcade.Sprite{
    constructor(_scene,_x,_y,_keyCache) {
        ...
        this.explosionRadius = this.scene.physics.add.sprite(this.x, this.y, 'joueur').setVisible(false).setOrigin(2*4,2*4).setCircle(64*4);
        this.explosionRadius.body.allowGravity = false;
        this.explosionRadiusSprite = this.scene.physics.add.sprite(this.x, this.y, 'radiusExplo').setVisible(true).setOrigin(0.5*4,0.5*4).setScale(4*4,4*4).setAlpha(0.5);
    ...
    destroySelf() {
        ...
        this.scene.cam.shake(100,0.5/(distance*(0.2*4)), false)
